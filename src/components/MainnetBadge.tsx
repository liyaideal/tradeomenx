 ...
 */
export const MainnetBadge = ({ className, size = "sm" }: MainnetBadgeProps) => {
  const textSize = size === "md" ? "text-[10px]" : "text-[9px]";
  return (
    <span
      className={cn(
        "inline-flex items-center gap-1 rounded-sm border border-trading-green/40 bg-trading-green/10 px-1.5 py-0.5 font-mono uppercase tracking-wider text-trading-green",
        textSize,
        className,
      )}
