...
| 日期 | 文档 | 摘要 |
|---|---|---|
| 2026-07-21 | [Pro 现货全站渗透 · 轮次 1](./2026-07-21-spot-dataline-hardening.md) | 共享数据链路把现货当合约的地方全部堵死：`DesktopPositionsPanel` 只展示 futures 行；`useSupabaseOrders.fillOrder/cancelOrder` 按 `product_line` 分流（spot 走 `fillSpotLimitOrder`/`cancelSpotLimitOrder`，spot short 显式抛错）；`PositionDetailContent` spot 分支隐藏 leverage/liq/funding，改用 Shares/Avg cost/Current value；Portfolio View 按行分流到 `/spot?event={id}`；仓位券排除现货（`EventPickerList` 过滤 + `redeem-position-voucher` 服务端返回 409）；`backend-boundary.md` 追加 2026-07-21 章节；`/style-guide` Spot 新增 Position detail 双端对照 |
|---|---|---|
