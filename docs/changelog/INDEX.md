...
| 日期 | 文档 | 摘要 |
|---|---|---|
| 2026-07-21 | [Pro/Lite 双 surface 骨架 + Stocks 快速买入（Round A）](./2026-07-21-lite-mode-structure.md) | 首次消费 `profiles.preferred_surface`（此前沉睡）：新增 `SurfaceContext` + 顶栏/移动 drawer `Switch to Pro/Lite` 入口；仅 Home / Events / `/spot` 三处 fork（其余共享，Pro 版一行未改）；Lite Home 板块 rail（Sports 外链常驻 + Stocks/Crypto/Macro/Entertainment 按活跃事件动态生成）；Lite `LiteBuyPanel` = marketable limit 封装：报价切边时快照 `quotedPrice` + 提交时读模拟盘口 `bestAsk`（与 Pro `/spot` `book.asks[0]` 同源）+ $0.02 滑点上限，超限报 `Price moved, try again`，成交后按 `balanceDelta` 走 `deductBalance`（Trial-first）；下单门控并入 `isPastFreeze`；`side_labels` 缺失时 fallback `Yes/No`；全 Lite surface 禁用 Margin/Liquidation/Funding/Leverage/Long/Short/Spot/Futures 文案；`/style-guide` 新增 Lite Mode tab 直接渲染真实 `LiteBuyPanel` 双端穷举 7 态；无 schema 变更；Boost 面板与 Sports 表单对齐属后续轮次 |
