...
ALTER TABLE public.position_vouchers
  ...
  CHECK (status = ANY (ARRAY['granted'::text, 'issued'::text, 'claimed'::text, 'redeemed'::text, 'settled'::text, 'expired'::text, 'revoked'::text]));
...
SET status = 'claimed',
    claimed_at = COALESCE(claimed_at, issued_at)
WHERE status = 'issued'
...
BEGIN
  ...
  -- ========== POSITION VOUCHERS (3 starter vouchers as 'granted' — user must claim) ==========
  INSERT INTO public.position_vouchers (user_id, code, face_value, status, issued_at, expires_at)
  VALUES
    (NEW.id, public.gen_voucher_code() || '01', 10, 'granted', now(), now() + interval '30 days'),
    (NEW.id, public.gen_voucher_code() || '02', 25, 'granted', now(), now() + interval '30 days'),
    (NEW.id, public.gen_voucher_code() || '03', 50, 'granted', now(), now() + interval '30 days');
