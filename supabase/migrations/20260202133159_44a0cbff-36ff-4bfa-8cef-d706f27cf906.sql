-- Drop the check constraint that's causing issues with settled positions (mark_price = 0 for lost positions)
ALTER TABLE public.positions DROP CONSTRAINT IF EXISTS check_positive_mark_price;

-- Re-create the handle_new_user function with valid mark_price values for settled positions
...
BEGIN
  ...
  -- Create demo SETTLED (Closed) positions with valid mark_price values (must be > 0)
  ...
  VALUES
    -- Won position - US Election (mark_price = 1.00 for winner)
    ...
    -- Won position - Super Bowl (mark_price = 1.00 for winner)
    ...
    -- Lost position - Fed Rate (mark_price = 0.01 instead of 0 to satisfy constraint)
