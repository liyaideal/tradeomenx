...
-- Update handle_new_user function to include mock pending + transaction history for all new users
...
BEGIN
  ...
  VALUES
    ...
    (NEW.id, 'deposit', 2500, 'processing', 'USDT Deposit', 'TRC20', '0x' || encode(gen_random_bytes(32), 'hex'), 12, 15, NOW() - INTERVAL '15 minutes'),
    (NEW.id, 'deposit', 1800, 'processing', 'USDT Deposit', 'ERC20', '0x' || encode(gen_random_bytes(32), 'hex'), 6, 12, NOW() - INTERVAL '25 minutes'),
    ...
    (NEW.id, 'withdraw', 2000, 'completed', 'USDT Withdrawal', 'TRC20', '0x' || encode(gen_random_bytes(32), 'hex'), 15, 15, NOW() - INTERVAL '4 days'),
    (NEW.id, 'withdraw', 1500, 'completed', 'USDT Withdrawal', 'ERC20', '0x' || encode(gen_random_bytes(32), 'hex'), 12, 12, NOW() - INTERVAL '2 days'),
