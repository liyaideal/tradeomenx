-- Fix handle_new_user trigger: use correct case for side ('long'/'short') and valid status values
...
BEGIN
  ...
  -- Create mock open positions (side must be lowercase: 'long' or 'short', status: 'Open' or 'Closed')
  ...
  -- Create mock closed positions (status must be 'Closed', not 'Settled')
  ...
  VALUES
    (NEW.id, 'Fed Interest Rate Decision', 'fed-rate-hold', 'Hold Steady', 'long', 0.65, 1.00, 200, 20, 10, 107.69, 538.46, 'Closed', now() - interval '1 day', now() - interval '5 days'),
    (NEW.id, 'Apple Stock Movement', 'aapl-up', 'Up >2%', 'long', 0.55, 0.00, 150, 15, 10, -150.00, -1000.00, 'Closed', now() - interval '2 days', now() - interval '6 days'),
    (NEW.id, 'Gold Price Weekly', 'gold-down', 'Down >1%', 'short', 0.42, 1.00, 100, 10, 10, 138.10, 1380.95, 'Closed', now() - interval '3 days', now() - interval '7 days');
