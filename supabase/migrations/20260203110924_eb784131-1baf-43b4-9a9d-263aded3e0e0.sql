-- Update handle_new_user function to fix balance calculation
-- The mock transactions now all use 'completed' status, so we need to recalculate
-- Current transactions:
-- Deposits: 2500 + 1800 + 5000 + 3000 + 2000 = 14,300
-- Withdrawals: -2000 - 1500 = -3,500
-- Trade profits: +850 + 1200 = +2,050
-- Trade losses: -320
-- Net = 14,300 - 3,500 + 2,050 - 320 = 12,530
...
BEGIN
  ...
  -- Calculate initial balance based on mock completed transactions:
  -- Deposits: 2500 + 1800 + 5000 + 3000 + 2000 = 14,300
  -- Withdrawals: -2000 - 1500 = -3,500
  -- Trade profits: +850 + 1200 = +2,050
  -- Trade losses: -320
  -- Net = 14,300 - 3,500 + 2,050 - 320 = 12,530
  v_initial_balance := 12530;
  
  -- Create user profile with calculated balance - use DO UPDATE to ensure balance is set
  ...
  -- Create mock transactions for new users (all completed for simpler accounting)
  ...
  VALUES
    -- RECENT DEPOSITS (completed - reflected in balance)
    (NEW.id, 'deposit', 2500, 'completed', 'USDT Deposit', 'TRC20', '0x' || replace(gen_random_uuid()::text, '-', '') || replace(gen_random_uuid()::text, '-', ''), 15, 15, NOW() - INTERVAL '15 minutes'),
    (NEW.id, 'deposit', 1800, 'completed', 'USDT Deposit', 'ERC20', '0x' || replace(gen_random_uuid()::text, '-', '') || replace(gen_random_uuid()::text, '-', ''), 12, 12, NOW() - INTERVAL '25 minutes'),
    
    -- TRANSACTION HISTORY (completed + failed - these are reflected in balance)
  ...
  VALUES
    ...
    (NEW.id, v_trade_id_6, 'Fed Rate Decision March 2025', 'No Cut', NULL, 'long', 0.72, 0.01, 1111.11, 8, 10, -80, -100, 'Closed', NOW() - INTERVAL '20 days', NOW() - INTERVAL '35 days');
