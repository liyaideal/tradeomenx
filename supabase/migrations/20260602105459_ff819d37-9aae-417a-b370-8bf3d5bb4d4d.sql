-- Helper: 6-char uppercase alphanumeric code (built from a UUID)
...
-- Seed 3 vouchers for every existing user that has none
INSERT INTO public.position_vouchers (
  user_id, code, face_value, status, issued_at, expires_at
)
SELECT
  p.user_id,
  public.gen_voucher_code() || lpad((row_number() OVER (PARTITION BY p.user_id))::text, 2, '0'),
  v.face_value,
  'issued',
  now(),
  now() + interval '7 days'
FROM public.profiles p
CROSS JOIN (VALUES (10::numeric), (25::numeric), (50::numeric)) AS v(face_value)
WHERE NOT EXISTS (
  SELECT 1 FROM public.position_vouchers pv WHERE pv.user_id = p.user_id
);

-- Update handle_new_user to also seed vouchers for new signups
...
BEGIN
  ...
  -- ========== POSITION VOUCHERS (3 starter vouchers) ==========
  INSERT INTO public.position_vouchers (user_id, code, face_value, status, issued_at, expires_at)
  VALUES
    (NEW.id, public.gen_voucher_code() || '01', 10, 'issued', now(), now() + interval '7 days'),
    (NEW.id, public.gen_voucher_code() || '02', 25, 'issued', now(), now() + interval '7 days'),
    (NEW.id, public.gen_voucher_code() || '03', 50, 'issued', now(), now() + interval '7 days');
