...
-- Update handle_new_user function to also create demo positions
...
BEGIN
  ...
  -- ========== CREATE INITIAL TRIAL BALANCE TRANSACTION ==========
  ...
  -- Create 3 active positions so users can see position management features
  ...
  -- Position 1: World Cup 2026 Winner - Long on Brazil (profitable)
  INSERT INTO public.trades (
    id, user_id, event_name, option_label, side, order_type, price, quantity, amount, margin, fee, status, pnl, leverage, created_at
  ) VALUES (
    v_trade_id_1, NEW.id, 'World Cup 2026 Winner', 'Brazil', 'buy', 'Market', 0.18, 500, 90, 90, 0.9, 'Filled', NULL, 10, now() - interval '2 days'
  );
  
  INSERT INTO public.positions (
    user_id, trade_id, event_name, option_label, side, entry_price, mark_price, size, margin, leverage, status, pnl, pnl_percent, tp_mode, sl_mode, created_at
  ) VALUES (
    NEW.id, v_trade_id_1, 'World Cup 2026 Winner', 'Brazil', 'long', 0.18, 0.22, 500, 90, 10, 'Open', 20, 22.22, '$', '$', now() - interval '2 days'
  );
  
  -- Position 2: Fed Rate Decision - Long on No Change (slightly losing)
  INSERT INTO public.trades (
    id, user_id, event_name, option_label, side, order_type, price, quantity, amount, margin, fee, status, pnl, leverage, created_at
  ) VALUES (
    v_trade_id_2, NEW.id, 'Fed interest rate decision January 2026?', 'No Change', 'buy', 'Market', 0.45, 200, 90, 90, 0.9, 'Filled', NULL, 10, now() - interval '1 day'
  );
  
  INSERT INTO public.positions (
    user_id, trade_id, event_name, option_label, side, entry_price, mark_price, size, margin, leverage, status, pnl, pnl_percent, tp_mode, sl_mode, created_at
  ) VALUES (
    NEW.id, v_trade_id_2, 'Fed interest rate decision January 2026?', 'No Change', 'long', 0.45, 0.42, 200, 90, 10, 'Open', -6, -6.67, '$', '$', now() - interval '1 day'
  );
  
  -- Position 3: ETH/BTC Ratio - Short position (profitable)
  INSERT INTO public.trades (
    id, user_id, event_name, option_label, side, order_type, price, quantity, amount, margin, fee, status, pnl, leverage, created_at
  ) VALUES (
    v_trade_id_3, NEW.id, 'ETH/BTC ratio end of Q1 2026?', '0.040 - 0.045', 'sell', 'Market', 0.35, 300, 105, 105, 1.05, 'Filled', NULL, 10, now() - interval '12 hours'
  );
  
  INSERT INTO public.positions (
    user_id, trade_id, event_name, option_label, side, entry_price, mark_price, size, margin, leverage, status, pnl, pnl_percent, tp_mode, sl_mode, created_at
  ) VALUES (
    NEW.id, v_trade_id_3, 'ETH/BTC ratio end of Q1 2026?', '0.040 - 0.045', 'short', 0.35, 0.32, 300, 105, 10, 'Open', 9, 8.57, '$', '$', now() - interval '12 hours'
  );
