...
## 16. 移动端列表页筛选规范（Mobile List Page Filter Pattern）
...
| 元素 | 规范 | 备注 |
|------|------|------|
| 筛选触发器 | `<Button variant="ghost" size="icon" className="h-9 w-9 rounded-full bg-muted/50">` + `<Filter />` 图标 | 使用 `MobileDrawer` 承载筛选内容 |
...
| Tab 导航 | 如果页面有 Tabs，Tabs 保持可见，与筛选按钮同行放置 | Tabs 左侧，Filter 按钮右侧 |

### 页面实现对照

| Page | Tabs | Filter Drawer | 组件 |
|------|------|---------------|------|
| EventsPage (`/events`) | `EventTabs`（All / Hot / Watchlist / Crypto / Macro） | Search + Expiry + Sort | `MobileActiveFilterDrawer` |
| ResolvedPage (`/resolved`) | 无 Tabs | Search + Time Range + Category + Sort | `MobileResolvedFilterDrawer` |

**禁止**：在移动端页面平铺 `<Select>`、`<Input>` 等筛选控件，必须收入 Drawer。
